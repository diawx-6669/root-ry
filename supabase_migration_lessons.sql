-- ══════════════════════════════════════════════════════════════
--  RootRy — миграция «Уроки дерева грамматики» (v3)
--  Запустить в Supabase Dashboard → SQL Editor
--  ПОСЛЕ schema.sql и supabase_migration_kspoya.sql
-- ══════════════════════════════════════════════════════════════

-- ── 1. Никнеймы: снимаем экранирование ───────────────────────────────
-- Раньше никнейм экранировался ПРИ ЗАПИСИ, и в базе лежало «Вася &amp; Петя».
-- Теперь экранирование делает только вывод, поэтому старые записи нужно
-- вернуть к исходному виду — иначе ученики так и будут видеть &amp; и &#39;.
UPDATE users
