...
// Голосовой режим добавляет к подсказке правила устного разговора и
// не трогает всё остальное: собеседник тот же, меняется только форма
// ответа. Режим — закрытый список, произвольную строку с клиента в
// подсказку подставить нельзя (см. AssistantChat).
func TestAssistantVoicePrompt(t *testing.T) {
	text := assistantPrompt("normal", "", "")
	voice := assistantPrompt("normal", "", "voice")

	if strings.Contains(text, "ГОЛОСОМ") {
		t.Error("правила голоса попали в обычную текстовую подсказку")
	}
	if !strings.Contains(voice, "ГОЛОСОМ") {
		t.Error("правила голоса не попали в голосовую подсказку")
	}
	if !strings.Contains(voice, `"reply"`) {
		t.Error("в голосовой подсказке потерялся формат ответа")
	}
	if !strings.Contains(voice, assistantLevels["normal"]) {
		t.Error("в голосовой подсказке потерялся уровень собеседника")
	}
	// В устной речи знаков препинания нет вовсе, поэтому пунктуационные
	// ошибки отмечать нечестно — об этом должно быть сказано прямо.
	if !strings.Contains(voice, "Пунктуационные ошибки") {
		t.Error("голосовая подсказка не запрещает считать ошибками пунктуацию")
	}
}
